"""
The definition is about the use of BM3D denoising under the PNP-ADMM-CNC framework
...
def analyze_parse_PNP_ADMM_CNC_BM3D(default_alpha, default_iter_num, default_lambda1, default_reo,default_b):
    ...
    PNP_ADMM_CNC_BM3D_opt = parser.parse_args()
    return PNP_ADMM_CNC_BM3D_opt

def PNP_ADMM_CNC_BM3D(mask, **PNP_ADMM_CNC_BM3D_opts):

    # ----------------------------------------
    # Preparation
    # ----------------------------------------

    alpha = PNP_ADMM_CNC_BM3D_opts.get('alpha', 1)
    iter_num = PNP_ADMM_CNC_BM3D_opts.get('iter_num', 20)
    lambda1 = PNP_ADMM_CNC_BM3D_opts.get('lambda1', 0.05)
    reo = PNP_ADMM_CNC_BM3D_opts.get('reo', 2.75)   # Here reo is 1/bata in the original article
    b = PNP_ADMM_CNC_BM3D_opts.get('b', 0.01)     # Here b is b^2 in the original article
    ...
    result_name = testset_name + '_' + task_current + 'PnP-ADMM-CNC-BM3D'
    ...
    for idx, img in enumerate(L_paths):
        ...
        if n_channels == 1:
            img_H = img_H.squeeze()
        if save_E:
            util.imsave(img_E, os.path.join(E_path, img_name + '_PNP_ADMM_CNC_BM3D.png'))
...
PNP_ADMM_CNC_BM3D_opt = analyze_parse_PNP_ADMM_CNC_BM3D(1.2, 50, 0.02, 0.6, 36)
...
with torch.no_grad():
    ...
    # ---- set options -----
    PNP_ADMM_CNC_BM3D_opts = dict(alpha=PNP_ADMM_CNC_BM3D_opt.alpha, iter_num=PNP_ADMM_CNC_BM3D_opt.iter_num,
                             lambda1=PNP_ADMM_CNC_BM3D_opt.lambda1, reo=PNP_ADMM_CNC_BM3D_opt.reo, b=PNP_ADMM_CNC_BM3D_opt.b)

    maskname = ['Q_Random30', 'Q_Radial30', 'Q_Cartesian30' ]

    k = 0
    print("------------------------------>model name = ({}) , mask = ({}) " .format('PNP_ADMM_CNC_BM3D', maskname[k]))
    out = PNP_ADMM_CNC_BM3D(mask[k], **PNP_ADMM_CNC_BM3D_opts)