"""
The definition is about the use of neural network denoising under the PNP-ADMM-L1 framework
...
def analyze_parse_PNP_ADMM_L1_D(default_iter_num, default_reo):
    ...
    PNP_ADMM_L1_D_opt = parser.parse_args()
    return PNP_ADMM_L1_D_opt

def PNP_ADMM_L1_D(model_name, mask, noises, **PNP_ADMM_L1_D_opts):

    # ----------------------------------------
    # Preparation
    # ----------------------------------------

    iter_num = PNP_ADMM_L1_D_opts.get('iter_num', 20)
    reo = PNP_ADMM_L1_D_opts.get('reo', 0.04)
    ...
    for idx, img in enumerate(L_paths):
        ...
        for i in range(iter_num):
            ...
            z = denoising_step1(model_name, x + w, x8, sigmas, i, model, noises, device, noise_level_model)
        ...
        if n_channels == 1:
            img_H = img_H.squeeze()
        if save_E:
            util.imsave(img_E, os.path.join(E_path, img_name + '_' + model_name + '_PNP_ADMM_L1_D.png'))
    ...
    logger.info('------> testset_name: ({}), Average PSNR:({:.3f})dB, Average ssim : ({:.3f}), Average re : ({:.3f}) )'.format(testset_name, ave_psnr, ave_ssim, ave_re))
...
PNP_ADMM_L1_D_opt1 = analyze_parse_PNP_ADMM_L1_D(50, 0.25) #fdncnn_gray
# the arguments are  default_max iteration, default_reo
PNP_ADMM_L1_D_opt2 = analyze_parse_PNP_ADMM_L1_D(50, 0.15) #dncnn_15
# the arguments are  default_max iteration,  default_reo
PNP_ADMM_L1_D_opt3 = analyze_parse_PNP_ADMM_L1_D(50, 0.25) #ffdnet_gray
# the arguments are  default_max iteration,  default_reo
PNP_ADMM_L1_D_opt4 = analyze_parse_PNP_ADMM_L1_D(50, 0.145) #ircnn_gray
# the arguments are  default_max iteration,  default_reo
PNP_ADMM_L1_D_opt5 = analyze_parse_PNP_ADMM_L1_D(50, 0.26) #drunet_gray
# the arguments are  default_max iteration,  default_reo
...
with torch.no_grad():
    ...
    # ---- set options -----
    PNP_ADMM_L1_D_opts1 = dict(iter_num=PNP_ADMM_L1_D_opt1.iter_num, reo=PNP_ADMM_L1_D_opt1.reo)
    PNP_ADMM_L1_D_opts2 = dict(iter_num=PNP_ADMM_L1_D_opt2.iter_num, reo=PNP_ADMM_L1_D_opt2.reo)
    PNP_ADMM_L1_D_opts3 = dict(iter_num=PNP_ADMM_L1_D_opt3.iter_num, reo=PNP_ADMM_L1_D_opt3.reo)
    PNP_ADMM_L1_D_opts4 = dict(iter_num=PNP_ADMM_L1_D_opt4.iter_num, reo=PNP_ADMM_L1_D_opt4.reo)
    PNP_ADMM_L1_D_opts5 = dict(iter_num=PNP_ADMM_L1_D_opt5.iter_num, reo=PNP_ADMM_L1_D_opt5.reo)

    name = ['fdncnn_gray', 'dncnn_15', 'ffdnet_gray', 'ircnn_gray', 'drunet_gray']
    maskname = ['Q_Random30', 'Q_Radial30', 'Q_Cartesian30']
    PNP_ADMM_L1_D_opts = [PNP_ADMM_L1_D_opts1, PNP_ADMM_L1_D_opts2, PNP_ADMM_L1_D_opts3, PNP_ADMM_L1_D_opts4, PNP_ADMM_L1_D_opts5]

    k = 0
    m = 4
    print("------------------------------>model name = ({}) , mask = ({}) " .format(name[m], maskname[0]))
    out = PNP_ADMM_L1_D(name[m], mask[k], noises, **PNP_ADMM_L1_D_opts[m])
